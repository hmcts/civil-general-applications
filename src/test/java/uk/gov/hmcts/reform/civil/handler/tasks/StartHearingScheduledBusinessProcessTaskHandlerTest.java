...
import static uk.gov.hmcts.reform.civil.handler.tasks.StartHearingScheduledBusinessProcessTaskHandler.BUSINESS_PROCESS;

@SpringBootTest(classes = {
    StartHearingScheduledBusinessProcessTaskHandler.class,
    JacksonAutoConfiguration.class,
    CaseDetailsConverter.class,
    StateFlowEngine.class
})
@ExtendWith(SpringExtension.class)
class StartHearingScheduledBusinessProcessTaskHandlerTest {
    ...
    @Autowired
    private StartHearingScheduledBusinessProcessTaskHandler handler;
    ...
    void init() {
        ...
        when(mockTask.getAllVariables()).thenReturn(Map.of(
            "caseId", CASE_ID,
            "caseEvent", START_HEARING_SCHEDULED_BUSINESS_PROCESS.name()
        ));
    }
    ...
    void shouldStartBusinessProcess_whenValidBusinessProcessStatus(BusinessProcessStatus status) {
        ...
        when(coreCaseDataService.startUpdate(CASE_ID, START_HEARING_SCHEDULED_BUSINESS_PROCESS))
            .thenReturn(startEventResponse);
        when(coreCaseDataService.submitGaUpdate(eq(CASE_ID), any(CaseDataContent.class))).thenReturn(caseData);

        handler.execute(mockTask, externalTaskService);

        verify(coreCaseDataService).startUpdate(CASE_ID, START_HEARING_SCHEDULED_BUSINESS_PROCESS);
