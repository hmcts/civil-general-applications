...
import static uk.gov.hmcts.reform.civil.callback.CaseEvent.START_GENERAL_APP_BUSINESS_PROCESS;
...
import static uk.gov.hmcts.reform.civil.handler.tasks.StartGeneralAppBusinessProcessTaskHandler.BUSINESS_PROCESS;

@SpringBootTest(classes = {
    StartGeneralAppBusinessProcessTaskHandler.class,
    JacksonAutoConfiguration.class,
    CaseDetailsConverter.class,
    StateFlowEngine.class
})
@ExtendWith(SpringExtension.class)
class StartGeneralAppBusinessProcessTaskHandlerTest {
    ...
    @Autowired
    private StartGeneralAppBusinessProcessTaskHandler handler;
    ...
    void init() {
        ...
        when(mockTask.getAllVariables()).thenReturn(Map.of(
            "caseId", CASE_ID,
            "caseEvent", START_GENERAL_APP_BUSINESS_PROCESS.name()
        ));
    }
    ...
    void shouldStartBusinessProcess_whenValidBusinessProcessStatus(BusinessProcessStatus status) {
        ...
        when(coreCaseDataService.startGaUpdate(CASE_ID, START_GENERAL_APP_BUSINESS_PROCESS))
            .thenReturn(startEventResponse);
        when(coreCaseDataService.submitGaUpdate(eq(CASE_ID), any(CaseDataContent.class))).thenReturn(caseData);

        handler.execute(mockTask, externalTaskService);

        verify(coreCaseDataService).startGaUpdate(CASE_ID, START_GENERAL_APP_BUSINESS_PROCESS);
