...
public class VerifyToProceedInHeritageEventCallbackHandler extends CallbackHandler {

    private final ApplicationEventPublisher applicationEventPublisher;

    private static final List<CaseEvent> EVENTS = singletonList(VERIFY_TO_PROCEED_IN_HERITAGE);

    private static  final List<CaseState> NON_LIVE_STATES =
        List.of(PROCEEDS_IN_HERITAGE, APPLICATION_CLOSED, ORDER_MADE, LISTING_FOR_A_HEARING, APPLICATION_DISMISSED);
    ...
    private CallbackResponse changeApplicationState(CallbackParams callbackParams) {
        ...
        if (!NON_LIVE_STATES.contains(caseData.getCcdState())) {
            ...
            applicationEventPublisher.publishEvent(new TakeApplicationOfflineEvent(caseId));
        }
